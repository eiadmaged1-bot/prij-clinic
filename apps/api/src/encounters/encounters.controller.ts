...
export class EncountersController {
  ...
  @Get(":id/readiness")
  @Permissions("encounter.read")
  readiness(@Param("id") id: string, @CurrentUser() user: AuthUser) {
    return this.encounters.readiness(id, user);
  }

  @Patch(":id/sign")
  @Permissions("encounter.sign")
  sign(@Param("id") id: string, @Body() dto: SignEncounterDto, @CurrentUser() user: AuthUser) {
    return this.encounters.sign(id, dto.expectedRevision, user);
  }
