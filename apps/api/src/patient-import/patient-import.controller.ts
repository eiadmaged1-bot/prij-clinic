...
export class PatientImportController {
  ...
  @Get() @Permissions("patient.create") list(@CurrentUser() user: AuthUser) { return this.imports.list(user); }
  ...
  @Post(":id/rollback") @Permissions("patient.create") rollback(@Param("id") id: string, @Body() dto: RollbackPatientImportDto, @CurrentUser() user: AuthUser) { return this.imports.rollback(id, dto, user); }
}
