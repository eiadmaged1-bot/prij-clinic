...
type Document = { id: string; title: string; organization: string; versionLabel?: string | null; guidelineStatus: string; reviewStatus: string; specialty: string; topic: string; fileMimeType?: string | null; downloadsAllowed?: boolean; source?: { name?: string; organization?: string }; sections?: Section[]; chunks?: Chunk[] };
  ...
  return <AppShell>
    ...
    <section className="guideline-pdf-workspace" ref={viewerRef}>
      ...
      <aside className={`panel guideline-summary-citations ${mobileTab === "Clinical Summary" || mobileTab === "Sources" ? "mobile-active" : ""}`}>
        {mobileTab === "Sources" ? <><h2>Sources</h2><p>{document.source?.organization || document.source?.name || document.organization}</p><p>{document.specialty} · {document.topic}</p><p>Status: {document.reviewStatus}</p></> : <><h2>Clinical Summary</h2><span className="badge warning">Needs doctor review</span><p className="muted">No clinic-approved structured summary is published for this document yet. Read the authoritative PDF and cited source sections.</p></>}
