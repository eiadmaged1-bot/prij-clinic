...
import { PdfCanvasViewer } from "../../../components/guidelines/PdfCanvasViewer";
...
export default function GuidelineViewerPage() {
  ...
  const indexedSections = useMemo(() => document?.sections?.length ? document.sections : (document?.chunks ?? []).map((chunk) => ({ id: chunk.id, heading: chunk.heading || chunk.sectionPath || `Section ${chunk.chunkIndex + 1}`, orderIndex: chunk.chunkIndex, text: chunk.text, pageStart: chunk.pageStart, pageEnd: chunk.pageEnd })), [document]);
  ...
  return <AppShell>
    ...
    {mobileTab === "Related Protocols" ? <section className="panel"><h2>Related protocols</h2><p className="muted">Search Protocol Atlas using this document topic. Applying a protocol remains a separate Doctor-reviewed action.</p><Link className="button secondary" href={`/protocol-atlas?query=${encodeURIComponent(document.topic)}`}>Search related protocols</Link></section> : null}
    {mobileTab === "Notes" ? <section className="panel"><h2>Notes</h2><p className="empty-state compact">Document note storage is not configured. Notes cannot be saved from this viewer.</p><button className="button secondary" type="button" disabled title="Document note storage is not configured">Add note unavailable</button></section> : null}
    {mobileTab === "Version History" ? <section className="panel"><h2>Version history</h2>{document.versions?.length ? <div className="data-list">{document.versions.map((version) => <article className="data-row" key={version.id}><strong>{version.versionLabel}</strong><span>{version.publishedYear ?? version.publicationDate?.slice(0, 10) ?? "Date not recorded"}</span><span className="badge">{version.status}</span></article>)}</div> : <p className="empty-state compact">No separate historical version assets are stored for this document.</p>}</section> : null}
    {["PDF", "Clinical Summary", "Sections", "Sources"].includes(mobileTab) ? <section className="guideline-pdf-workspace" ref={viewerRef}>
      ...
      <main className={`panel guideline-pdf-stage ${mobileTab === "PDF" ? "mobile-active" : ""}`} onTouchStart={(event) => setTouchStart(event.touches[0]?.clientX ?? null)} onTouchEnd={(event) => swipeEnd(event.changedTouches[0]?.clientX ?? 0)}>
        <div className="pdf-toolbar" aria-label="PDF controls">
          <button type="button" aria-label="Previous page" onClick={() => goToPage(page - 1)} disabled={page <= 1}>‹</button><span className="pdf-page-count" aria-live="polite">Page {page}/{pageCount ?? "…"}</span><button type="button" aria-label="Next page" onClick={() => goToPage(page + 1)} disabled={!pageCount || page >= pageCount}>›</button>
          <button type="button" aria-label="Zoom out" onClick={() => { setFit("custom"); setZoom((value) => Math.max(50, value - 10)); }}>−</button><span className="pdf-zoom-value">{zoom}%</span><button type="button" aria-label="Zoom in" onClick={() => { setFit("custom"); setZoom((value) => Math.min(200, value + 10)); }}>+</button><details className="pdf-more-menu"><summary aria-label="More PDF actions">⋯</summary><button type="button" onClick={() => { setFit("width"); setZoom(100); }}>Fit width</button><button type="button" onClick={() => { setFit("page"); setZoom(100); }}>Fit page</button><button type="button" onClick={() => setRotation((value) => (value + 90) % 360)}>Rotate</button><button type="button" onClick={() => void viewerRef.current?.requestFullscreen()}>Full screen</button><button type="button" onClick={() => window.open(`${getApiBaseUrl()}/guidelines/documents/${encodeURIComponent(params.id)}/view`, "_blank", "noopener,noreferrer")}>Open original</button></details>
        </div>
...
function GuidelinePageFallback({ page, sections }: { page: number; sections: Section[] }) {
  if (sections.length === 0) {
    return (
      <article className="guideline-page-fallback unavailable-source-mode" aria-label="Unavailable source mode">
        <span className="badge warning">Unavailable Source Mode</span>
        <p>No authoritative asset and no extracted text are available for this record.</p>
        <p className="muted">This document exists as metadata only. You must rely on external official sources.</p>
      </article>
    );
  }

  const content = sections.filter((section) => page >= (section.pageStart ?? 1) && page <= (section.pageEnd ?? section.pageStart ?? 1));
  return <article className="guideline-page-fallback extracted-text-mode" aria-label={`Extracted fallback for page ${page}`}><span className="badge">Extracted Text Mode (Page {page})</span>{content.length ? content.map((section) => <section key={section.id}><h2>{section.heading}</h2><div className="guideline-readable-text" dir="auto">{section.text || "No extracted text for this section."}</div></section>) : <p>No extracted page text is available for this exact page. Please browse other sections.</p>}</article>;
}
