...
export default function ObUltrasoundsPage() {
  ...
  return <AppShell>
    ...
    <section className="content-grid"><article className="panel"><div className="section-heading"><div><h2>{t("previousScans")}</h2><p className="muted">Server-paginated OB, gynecology, and fertility imaging workflow.</p></div><span className="badge">{status}</span></div>
      ...
      <div className="table-scroll"><table><thead><tr><th>Patient / MRN</th><th>Context</th><th>Scan</th><th>GA / cycle</th><th>Date</th><th>Doctor</th><th>Completeness</th><th>Status</th><th>Action</th></tr></thead><tbody>{scans.map((scan) => <tr key={scan.id}><td><strong>{[scan.patient?.firstName, scan.patient?.lastName].filter(Boolean).join(" ") || "Patient record"}</strong><br /><span className="muted">{scan.patient?.medicalRecordNumber || "MRN unavailable"}</span></td><td>{scan.clinicalContext ?? "OB"}</td><td>{scan.scanType || "Not set"}</td><td>{scan.clinicalContext === "FERTILITY" ? `Cycle day ${scan.cycleDay ?? "not recorded"}` : scan.gestationalAgeDisplay || "Not recorded"}</td><td>{formatDate(scan.performedAt)}</td><td>{scan.createdByUser?.displayName || "Operator not shown"}</td><td><span className="badge">{scanCompleteness(scan)}</span></td><td><span className="badge">{scan.status.replaceAll("_", " ")}</span></td><td><Link className="button secondary compact" href={`/patients/${encodeURIComponent(scan.patientId)}/ultrasounds/${encodeURIComponent(scan.id)}`}>{t("openPatientUltrasound")}</Link></td></tr>)}{!scans.length ? <tr><td colSpan={9}><p className="empty-state compact">{t("noScansMatch")}</p></td></tr> : null}</tbody></table></div>
