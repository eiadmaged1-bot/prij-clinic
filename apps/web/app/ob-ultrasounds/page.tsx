...
export default function ObUltrasoundsPage() {
  ...
  return <AppShell>
    ...
    <section className="content-grid ultrasound-center-grid"><article className="panel"><div className="section-heading"><div><h2>{t("previousScans")}</h2><p className="muted">{t("serverPaginatedImaging")}</p></div><span className="badge">{status === "loading" ? t("loadingScans") : status === "unavailable" ? t("ultrasoundHistoryUnavailable") : status === "empty" ? t("noScansPermitted") : t("ready")}</span></div>
      ...
      <div className="toolbar"><label>{t("patientMrnScanType")}<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("refineScanHistory")} /></label><label>{t("patientType")}<select value={patientTypeFilter} onChange={(event) => { setPatientTypeFilter(event.target.value); setPage(1); }}><option value="">{t("allPatientTypes")}</option><option value="OBSTETRIC">{t("obstetric")}</option><option value="HIGH_RISK_OBSTETRIC">{t("highRiskObstetric")}</option><option value="GYNECOLOGY">{t("gynecology")}</option><option value="INFERTILITY">{t("infertility")}</option><option value="POSTPARTUM">{t("postpartum")}</option><option value="PREVENTIVE_WELL_WOMAN">{t("preventive")}</option><option value="OTHER">{t("other")}</option></select></label><label>{t("context")}<select value={contextFilter} onChange={(event) => { setContextFilter(event.target.value); setPage(1); }}><option value="">{t("obGynFertility")}</option><option value="OB">{t("obstetric")}</option><option value="GYN">{t("gynecology")}</option><option value="FERTILITY">{t("infertility")}</option></select></label><label>{t("doctorOperator")}<input value={doctorFilter} onChange={(event) => setDoctorFilter(event.target.value)} /></label><label>{t("branch")}<input value={branchFilter} onChange={(event) => setBranchFilter(event.target.value)} /></label><label>{t("date")}<input type="date" value={dateFilter} onChange={(event) => { setDateFilter(event.target.value); setPage(1); }} /></label><button className="button secondary compact" type="button" onClick={() => { setPage(1); void load(); }}>{t("search")}</button></div>
