...
export default function DoctorCaseLibraryPage() {
  ...
  const [filters, setFilters] = useState({ search: "", dateRange: "month", visitType: "", doctorId: "" });
    ...
    <AppShell>
      ...
      <section className="panel compact-panel">
        <form className="case-library-filter-grid" onSubmit={submit}>
          ...
          <label>Visit type<select value={filters.visitType} onChange={(event) => setFilters((current) => ({ ...current, visitType: event.target.value }))}><option value="">All cases</option><option value="كشف">كشف</option><option value="إعادة">إعادة</option><option value="استشارة">استشارة</option><option value="مستعجل">مستعجل</option></select></label>
      ...
      <section className="case-library-grid">
        ...
        {cases.map((clinicCase) => (
          <article className="data-row case-library-card" key={clinicCase.id} style={{ borderLeftColor: clinicCase.doctorSignature.doctorColor }}>
            ...
            <p className="muted">{clinicCase.visitType} - {new Date(clinicCase.visitDateTime).toLocaleString()} - {clinicCase.summaryPreview}</p>
            <div className="tag-row">{clinicCase.tags.map((tag) => <span className="badge" key={tag}>{tag}</span>)}</div>
