...
// ==========================================
// REGISTRY MODULE RENDERER
// ==========================================

export const PatientSummaryComponents: Record<string, React.FC<any>> = {
  "overview": ({ patient, related, timelineItems }) => <Overview patient={patient} related={related} timelineItems={timelineItems} />
};

export const TimelineModuleComponents: Record<string, React.FC<any>> = {
...
export const ClinicalPhasesModuleComponents: Record<string, React.FC<any>> = {
  "gynecology": ({ patient, related }) => <GynecologyWorkspace patient={patient} visits={(related.gynecology ?? []) as any[]} />,
  ...
  "mother-baby": ({ related }) => <MotherBabyWorkspace pregnancies={(related.pregnancy ?? []) as any[]} reports={related.files ?? []} orders={related.orders ?? []} />,
  "pregnancy": ({ patient, related }) => (
    <>
      <SmartHistoryOptionChips patient={patient} />
      <SmartObHistoryTags />
      <ObDatingReviewPanel patient={patient} pregnancies={(related.pregnancy ?? []) as any[]} />
      <ObgynWorkspace patient={patient} pregnancies={(related.pregnancy ?? []) as any[]} reports={related.files ?? []} orders={related.orders ?? []} />
    </>
  ),
  "ultrasound": ({ patient, related }) => <UltrasoundWorkspace patient={patient} pregnancies={(related.pregnancy ?? []) as any[]} reports={related.files ?? []} orders={related.orders ?? []} />
};

export const OtherWorkspaceComponents: Record<string, React.FC<any>> = {
  "case-boards": ({ patient, related }) => <CaseBoardsPanel patient={patient} related={related} />,
  "history": ({ patient, related, submitPatientAction, actionStatus }) => (
    <>
...
export const WorkspaceComponentsRegistry = {
  ...PatientSummaryComponents,
  ...TimelineModuleComponents,
  ...ClinicalPhasesModuleComponents,
  ...OtherWorkspaceComponents
};

export function WorkspaceModuleRenderer(props: any) {
  const Component = WorkspaceComponentsRegistry[props.active.key];
  if (Component) return <Component {...props} />;
  
  const ignoredKeys = [
    "overview", "case-feed", "case-boards", "mother-baby", "medical", "history-sheet", "care-assist", 
    "doctor-visit", "clinical", "timeline", "print-packet", "ai-snapshot", "protocol-atlas", "calculators", 
    "pregnancy", "ultrasound", "medications", "allergies", "herbals", "medication-safety", "prescription-safety"
  ];
  
  if (!ignoredKeys.includes(props.active.key)) {
    return <RelatedPanel config={props.active} rows={props.related[props.active.key] ?? []} />;
  }
  return null;
}
