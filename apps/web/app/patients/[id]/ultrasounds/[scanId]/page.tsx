...
type Scan = {
  ...
  comparisonSourceScanId?: string | null;
...
type StructuredFindings = {
  ...
  highRisk?: { growthTrend?: string; fetalSurveillance?: string; nstCtgReference?: string; maternalVitalsReference?: string; riskFactors?: string; deliveryPlanningReference?: string };
};
...
export default function UltrasoundEditorPage() {
  ...
  const [previousScans, setPreviousScans] = useState<Scan[]>([]);
  ...
  useEffect(() => { void loadPreviousScans(); }, [patientId, scan?.id]);
  ...
  async function loadPreviousScans() {
    const response = await api(`/ob-ultrasounds?patientId=${encodeURIComponent(patientId)}&page=1&limit=50`).catch(() => null);
    if (!response?.ok) return;
    const body = await response.json() as { obUltrasounds?: Scan[] };
    setPreviousScans((body.obUltrasounds ?? []).filter((item) => item.id !== scan?.id && ["signed", "final", "amended"].includes(item.status)));
  }
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const structuredFindingsJson: StructuredFindings = {
      ...
      highRisk: compact({ growthTrend: text(form, "growthTrend"), fetalSurveillance: text(form, "fetalSurveillance"), nstCtgReference: text(form, "nstCtgReference"), maternalVitalsReference: text(form, "maternalVitalsReference"), riskFactors: text(form, "riskFactors"), deliveryPlanningReference: text(form, "deliveryPlanningReference") })
    };
    const payload = compact({
      ...
      dopplerNote: text(form, "dopplerNote"), impressionText: text(form, "impressionText"), comparisonText: text(form, "comparisonText"), comparisonSourceScanId: text(form, "comparisonSourceScanId") || undefined, structuredFindingsJson
    });
  ...
  return <AppShell>
    ...
    <section className="ultrasound-editor-grid">
      ...
      <main className="panel printable-summary">
        ...
        <form key={scan?.id ?? "new"} className="obgyn-form-grid grouped" onSubmit={save}>
          ...
          <fieldset className="obgyn-fieldset wide"><legend>Comparison and impression</legend><label>Previous signed source scan<select name="comparisonSourceScanId" defaultValue={scan?.comparisonSourceScanId ?? ""} disabled={immutable}><option value="">No source scan selected</option>{previousScans.map((item) => <option key={item.id} value={item.id}>{item.scanType || "Ultrasound"} · {new Date(item.performedAt).toLocaleDateString()} · {item.status}</option>)}</select></label><label>Measured comparison<textarea name="comparisonText" defaultValue={scan?.comparisonText ?? ""} disabled={immutable} placeholder="Record measured change from the selected signed source; do not infer significance." /></label><label>Doctor-written impression<textarea name="impressionText" defaultValue={scan?.impressionText ?? ""} disabled={immutable} /></label></fieldset>
...
function SecureGallery({ patientId, documents, api }: { patientId: string; documents: PatientDocument[]; api: (path: string, init?: RequestInit) => Promise<Response> }) {
  ...
  useEffect(() => { if (activeIndex === null) return; const onKey = (event: KeyboardEvent) => { if (event.key === "Escape") setActiveIndex(null); if (event.key === "ArrowLeft") setActiveIndex((value) => value === null ? null : (value - 1 + documents.length) % documents.length); if (event.key === "ArrowRight") setActiveIndex((value) => value === null ? null : (value + 1) % documents.length); }; window.addEventListener("keydown", onKey); return () => window.removeEventListener("keydown", onKey); }, [activeIndex, documents.length]);
  if (!documents.length) return <p className="empty-state compact">No secured images attached.</p>;
  const currentIndex = activeIndex ?? 0;
  const active = activeIndex === null ? null : documents[currentIndex];
  return <><div className="ultrasound-gallery">{documents.map((document, index) => urls[document.id] ? <button key={document.id} type="button" onClick={() => setActiveIndex(index)} aria-label={`Open ${document.title} fullscreen`}><img src={urls[document.id]} alt={document.title} /><span>{document.title}</span></button> : <span key={document.id} className="muted">Loading {document.title}</span>)}</div>{active && urls[active.id] ? <div className="ultrasound-lightbox" role="dialog" aria-modal="true" aria-label="Ultrasound image viewer"><button type="button" className="ultrasound-lightbox-close" onClick={() => setActiveIndex(null)} aria-label="Close image viewer">×</button><button type="button" onClick={() => setActiveIndex((currentIndex - 1 + documents.length) % documents.length)} aria-label="Previous image">‹</button><figure><img src={urls[active.id]} alt={active.title} /><figcaption>{active.title} · {currentIndex + 1} of {documents.length}</figcaption></figure><button type="button" onClick={() => setActiveIndex((currentIndex + 1) % documents.length)} aria-label="Next image">›</button></div> : null}</>;
}
