  ...
  return (
    <AppShell>
      ...
      <section className="panel">
        ...
        {filtered.length > 0 ? <><div className="table-scroll patient-directory-desktop-table"><table><thead><tr><th>Patient</th><th>File number</th><th>Phone</th><th>Age / DOB</th><th>Type</th><th>Branch</th><th>Last visit</th><th>Queue</th><th>Important tags</th><th>Actions</th></tr></thead><tbody>{filtered.map((patient) => <tr key={patient.id}><td><strong>{patientDisplayName(patient)}</strong></td><td>{patientFileNumber(patient)}</td><td>{patient.phone || "Not recorded"}</td><td>{patient.dateOfBirth ? `${ageLabel(patient.dateOfBirth)} · ${patient.dateOfBirth.slice(0, 10)}` : patient.yearOfBirth ? `Born ${patient.yearOfBirth}` : "Not recorded"}</td><td>{patientTypeLabel(patient.patientType)}</td><td>{patient.branch?.name ?? "Unavailable"}</td><td>{patient.latestVisitDate ? patient.latestVisitDate.slice(0, 10) : "No visit"}</td><td>{patient.queueState ? `#${patient.queueState.queueNumber} · ${friendlyStatus(patient.queueState.status)}` : "Not queued"}</td><td>{phaseTypeLabel(patient.currentPhase?.phaseType)}</td><td><div className="table-row-actions"><Link className="button secondary compact" href={`/patients/${patient.id}`}>Open</Link><Link className="button secondary compact" href={`/reception/check-in?patientId=${patient.id}`}>Queue</Link><button className="button secondary compact" type="button" aria-pressed={Boolean(patient.favorited)} aria-label={patient.favorited ? `Remove ${patientDisplayName(patient)} from favorites` : `Add ${patientDisplayName(patient)} to favorites`} onClick={() => void toggleFavorite(patient)}>{patient.favorited ? "★" : "☆"}</button></div></td></tr>)}</tbody></table></div><div className="form-actions patient-directory-desktop-pagination" aria-label="Patient directory pagination"><button className="button secondary compact" type="button" disabled={page <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>Previous</button><span className="muted">Page {pageInfo.page} of {Math.max(1, Math.ceil(pageInfo.total / pageInfo.limit))}</span><button className="button secondary compact" type="button" disabled={!pageInfo.hasMore} onClick={() => setPage((value) => value + 1)}>Next</button></div></> : null}
