...
export default function PatientsPage() {
  ...
  const advancedFilterCount = [patientType !== "all", branchId !== "all", sortMode !== "created_newest"].filter(Boolean).length;
  ...
  return (
    <AppShell>
      <section className="page-header calm-directory-header">
        <div className="header-row">
          ...
          <div className="form-actions">
            {user?.roles.some((role) => ["Owner", "Admin"].includes(role)) ? <Link className="button secondary" href="/patients/import">{t("importCsvXlsx")}</Link> : null}
            <Link className="button" href="/patients/new"><ThreeDMedicalIcon name="patients" size="sm" />{t("newPatientFile")}</Link>
          </div>
        </div>
      </section>
      ...
      <section className="panel calm-directory-panel">
        <div className="section-heading calm-directory-heading">
          <div>
        ...
        <div className="calm-directory-search-row">
          <label className="calm-directory-search">
            <span>{t("patientSearchLabel")}</span>
            <input onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder={t("patientSearchPlaceholder")} value={query} />
          </label>
          <label>
          ...
          <button className="button secondary compact" onClick={() => void loadPatients(query.trim(), page)} type="button"><ThreeDMedicalIcon name="search" size="sm" tone="slate" />{t("refresh")}</button>
        </div>

        <details className="calm-directory-filters">
          <summary><span>Filters and sorting</span>{advancedFilterCount ? <span className="badge">{advancedFilterCount}</span> : <span className="muted">Optional</span>}</summary>
          <div className="calm-directory-filter-grid">
            <label>{t("sortBy")}<select onChange={(event) => { setSortMode(event.target.value); setPage(1); }} value={sortMode}>
              <option value="created_newest">{t("createdNewest")}</option><option value="created_oldest">{t("createdOldest")}</option><option value="name_az">{t("nameAz")}</option><option value="name_za">{t("nameZa")}</option><option value="file_number">{t("fileNumber")}</option><option value="last_visit_desc">{t("lastVisitRecent")}</option><option value="age_year">{t("ageYearBirth")}</option>
            </select></label>
            <label>{t("patientType")}<select onChange={(event) => { setPatientType(event.target.value); setPage(1); }} value={patientType}><option value="all">{t("allPatientTypes")}</option>{patientTypeOptions.map((option) => <option key={option.value} value={option.value}>{patientTypeDisplay(option.value, language)}</option>)}</select></label>
            <label>{t("branch")}<select onChange={(event) => { setBranchId(event.target.value); setPage(1); }} value={branchId}><option value="all">{t("allPermittedBranches")}</option>{branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></label>
            <button className="button secondary compact" type="button" onClick={clearFilters}>{t("clearFilters")}</button>
          </div>
        </details>

        {error ? <p className="form-error">{error}</p> : null}
        {status === "Login required" ? <div className="empty-state">{t("signInBeforePatients")} <Link href="/login">{t("goToLogin")}</Link></div> : null}
        {status === "Loading" ? <div className="skeleton" /> : null}
        {status !== "Loading" && filtered.length === 0 && status !== "Login required" ? <div className="empty-state smart-empty-state"><ThreeDMedicalIcon name="files" size="sm" tone="slate" /><span>{t("noPatientFilesMatch")}</span><Link className="button secondary compact" href="/patients/new">{t("newPatientFile")}</Link></div> : null}

        {filtered.length > 0 ? <div className="patient-result-list">
          {filtered.map((patient) => <article className="patient-result-card" key={patient.id}>
          ...
          <div className="form-actions" aria-label={t("patientDirectory")}><button className="button secondary compact" type="button" disabled={page <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>{t("previous")}</button><span className="muted">{t("page")} {pageInfo.page} {t("of")} {Math.max(1, Math.ceil(pageInfo.total / pageInfo.limit))}</span><button className="button secondary compact" type="button" disabled={!pageInfo.hasMore} onClick={() => setPage((value) => value + 1)}>{t("next")}</button></div>
        </div> : null}
      </section>
    </AppShell>
  );
...
function patientDisplayName(patient: Patient) { const name = `${patient.firstName} ${patient.lastName}`.trim(); return name || "Patient file"; }
function compactAge(patient: Patient) { if (patient.dateOfBirth) return ageLabel(patient.dateOfBirth).replace(/^Age\s*/i, "").replace(/\s*years?$/i, "") + "y"; return patient.yearOfBirth ? `${new Date().getUTCFullYear() - patient.yearOfBirth}y` : "Age —"; }
