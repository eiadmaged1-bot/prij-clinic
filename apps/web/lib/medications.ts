...
export type PharmacologyAtlasRoom = { name: string; icon: string; familyCount: number; genericCount: number; exampleFamilies: string[]; families: PharmacologyAtlasFamily[]; incompleteFamilies: Array<Omit<PharmacologyAtlasFamily, "generics">> };
export type PharmacologyAtlas = { rooms: PharmacologyAtlasRoom[]; familyDirectory: Array<{ id: string; code: string; name: string; genericCount: number; coverage: string }>; allGenerics: PharmacologyAtlasGeneric[]; unlinkedGenerics: PharmacologyAtlasGeneric[]; recentlyReviewed: PharmacologyAtlasGeneric[]; totals: { families: number; generics: number; linkedGenerics: number; unlinkedGenerics: number; unlinkedRate: number; populatedRooms: number; familiesBeingCompleted: number }; browseViews: string[]; completeDatasetClaimed: false };
