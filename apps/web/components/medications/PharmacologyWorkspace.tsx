...
export function PharmacologyWorkspace() {
  ...
  useEffect(() => {
    ...
    void Promise.all([getPharmacologyAtlas(), getPharmacologyCoverage()]).then(([atlasData, coverageData]) => {
      ...
      setStatus(`${atlasData.totals.generics} active generics across ${atlasData.totals.families} active families; ${atlasData.totals.officialProfiles} official profiles. Missing sections remain explicit.`);
  ...
  return <section className="pharmacology-workspace">
    ...
    {selected ? <div className="pharmacology-profile-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeProfile(); }}><aside className="pharmacology-profile-panel" role="dialog" aria-modal="true" aria-label={`${selected.genericName} pharmacology profile`}>
      ...
      {selected.officialProfile?.publicationState === "SOURCE_CONFLICT" ? <div className="typed-state warning" role="alert"><strong>Authoritative label conflict</strong><p>Current official product labels differ in one or more safety sections. The cards below show the named representative label only; review its source and alternatives before clinical use. No unified interpretation was generated.</p></div> : null}
      {selected.officialProfile?.publicationState === "SOURCE_INCOMPLETE" ? <div className="typed-state warning"><strong>Source incomplete</strong><p>Only available official sections are shown. Missing fields were not inferred.</p></div> : null}
...
function sectionItems(profile: PharmacologyProfile, section: typeof profileSections[number]) {
  ...
  if (section === "Uses") return profile.officialProfile?.indications ?? [];
  if (section === "Mechanism") return profile.officialProfile?.mechanism?.length ? profile.officialProfile.mechanism : profile.mechanism ?? [];
  ...
  if (section === "Renal/hepatic") return profile.officialProfile ? [...profile.officialProfile.renal, ...profile.officialProfile.hepatic] : [...flattenRecords(profile.renal), ...flattenRecords(profile.hepatic)];
