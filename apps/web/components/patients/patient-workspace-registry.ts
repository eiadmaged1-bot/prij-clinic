...
import type { CanonicalPatientType } from "@/lib/patient-labels";
...
export type PatientWorkspaceItem = {
  ...
  permittedRoles: string[]; applicablePatientContexts: CanonicalPatientType[];
...
export const patientWorkspaceRegistry: PatientWorkspaceItem[] = [
  item("overview", "Patient summary", "ملخص المريضة", 10, "patients", [], ["patient.read"], true, true, loadLight),
  item("history", "History", "التاريخ المرضي", 15, "doctor", [], ["patient.read", "encounter.read"], true, false, loadHeavy, ["Owner", "Admin", "Doctor"], "/patients/:patientId/history-sheets", "historySheets"),
  item("doctor-visit", "Current visit", "الزيارة الحالية", 20, "encounter", ["start-visit", "continue-visit", "finish-visit"], ["encounter.read", "encounter.create"], true, true, loadLight, ["Owner", "Admin", "Doctor"]),
  ...
  item("medications", "Active medications", "الأدوية الحالية", 26, "prescription", [], ["patient_medications.read"], false, false, loadLight, ["Owner", "Admin", "Doctor"], "/patients/:patientId/medications", "medications", true),
  item("prescriptions", "Recent prescriptions", "الوصفات الحديثة", 30, "prescription", ["create-prescription"], ["prescription.read"], false, false, loadLight, ["Owner", "Admin", "Doctor"], "/prescriptions?patientId=:patientId", "prescriptions", true),
  item("investigations", "Investigations and results", "الفحوصات والنتائج", 40, "investigations", ["create-request", "review-result"], ["clinical_requests.read", "investigation.read"], false, false, loadLight, ["Owner", "Admin", "Doctor"], "/clinical-requests?patientId=:patientId", "clinicalRequests", true),
  item("pregnancy", "Pregnancy and women’s health", "الحمل وصحة المرأة", 50, "pregnancy", [], ["pregnancy.read", "pregnancy.manage"], false, false, loadHeavy, ["Owner", "Admin", "Doctor"], "/pregnancies?patientId=:patientId", "pregnancies", true),
  ...
  item("consents", "Documents and consent", "المستندات والموافقات", 100, "consent", [], ["patient.consent_read", "patient.consent_manage"], false, false, loadLight, undefined, "/consents?patientId=:patientId", "consentRecords", true),
  item("infertility", "Infertility and cycle monitoring", "الخصوبة ومتابعة الدورة", 110, "doctor", [], ["encounter.read"], false, false, loadHeavy, ["Owner", "Admin", "Doctor"], "/patients/:patientId/infertility", "cycles", true),
...
function contextsFor(key: string): CanonicalPatientType[] { if (key === "pregnancy") return ["OBSTETRIC", "HIGH_RISK_OBSTETRIC", "POSTPARTUM"]; if (key === "infertility") return ["INFERTILITY"]; return ["OBSTETRIC", "HIGH_RISK_OBSTETRIC", "GYNECOLOGY", "INFERTILITY", "POSTPARTUM", "PREVENTIVE_WELL_WOMAN", "OTHER"]; }
