...
export function PatientWorkspaceEditor({ patientId, permissions, roles, onApply }: { patientId: string; permissions: string[]; roles: string[]; onApply: (panels: WorkspacePanelPlacement[]) => void }) {
  ...
  return <>
    <section className="panel compact-panel patient-workspace-controls">
      <div className="section-heading"><div><h2>{t("modularWorkspace")}</h2><p className="muted">{status}</p></div><button className="button secondary compact" type="button" onClick={() => setEditing((value) => !value)}>{editing ? t("closeEditor") : t("editWorkspace")}</button></div>
      {editing ? <div className="workspace-editor-grid">
        <div className="inline-form"><label>{t("workspacePreset")}<select value={preset} onChange={(event) => void loadPreset(event.target.value)}>{["MINIMAL_VISIT", "GENERAL_WOMENS_HEALTH", "GYNECOLOGY", "INFERTILITY", "ROUTINE_OBSTETRICS", "HIGH_RISK_OBSTETRICS", "POSTPARTUM", "CUSTOM"].map((key) => <option key={key} value={key}>{key.replaceAll("_", " ")}</option>)}</select></label><label>{t("workspaceSaveScope")}<select value={scope} onChange={(event) => setScope(event.target.value as typeof scope)}><option value="PERSONAL">{t("workspaceMyAccount")}</option><option value="PATIENT">{t("workspaceThisPatient")}</option>{privileged ? <option value="CLINIC">{t("workspaceClinicDefault")}</option> : null}</select></label></div>
        <div className="workspace-panel-library">{panels.map((panel, index) => { const definition = patientWorkspaceRegistry.find((item) => item.key === panel.panelKey); if (!definition) return null; const panelLabel = language === "ar" ? definition.labelAr : definition.label; return <article className="data-row compact" key={panel.panelKey}><label><input type="checkbox" checked={!panel.hidden} disabled={definition.mandatory} onChange={(event) => update(panel.panelKey, { hidden: !event.target.checked })} /> {panelLabel}{definition.mandatory ? ` (${t("required")})` : ""}</label><div className="form-actions"><button type="button" className="button secondary compact" disabled={index === 0} onClick={() => move(index, -1)}>{t("moveUp")}</button><button type="button" className="button secondary compact" disabled={index === panels.length - 1} onClick={() => move(index, 1)}>{t("moveDown")}</button><select aria-label={`${panelLabel} size`} value={panel.size} disabled={definition.mandatory} onChange={(event) => update(panel.panelKey, { size: event.target.value as WorkspacePanelPlacement["size"] })}>{definition.supportedSizes.map((size) => <option key={size} value={size}>{size.toLowerCase()}</option>)}</select><label><input type="checkbox" checked={panel.collapsed} onChange={(event) => update(panel.panelKey, { collapsed: event.target.checked })} /> {t("collapsed")}</label><label><input type="checkbox" checked={panel.pinned} onChange={(event) => update(panel.panelKey, { pinned: event.target.checked })} /> {t("pinned")}</label></div></article>; })}</div>
        {scope !== "PERSONAL" ? <label>{t("reason")}<input value={reason} maxLength={500} onChange={(event) => setReason(event.target.value)} required /></label> : null}<div className="form-actions"><button className="button" type="button" onClick={() => void save()}>{t("saveLayout")}</button><button className="button secondary" type="button" onClick={() => void loadPreset("MINIMAL_VISIT")}>{t("resetPreview")}</button></div>
      </div> : null}
    </section>
    <section className="panel compact-panel missing-information-panel"><div className="section-heading"><div><h2>{t("missingInformation")}</h2><p className="muted">{t("deterministicPrompts")}</p></div>{findingsState === "ready" ? <span className="badge">{findings.length}</span> : null}</div>{findingsState === "loading" ? <div className="skeleton" /> : findingsState === "error" ? <p className="form-error">{t("missingRulesLoadError")}</p> : findings.length ? <div className="dense-card-list">{findings.map((finding) => <article className="data-row compact" key={finding.key}><div><strong>{finding.missingItem}</strong><span className="badge warning">{finding.state}</span></div><p>{finding.reason}</p><small>{finding.requirement} · {finding.context} · rule {finding.ruleVersion}</small><a className="button secondary compact" href={finding.actionLink}>{t("recordOrReview")}</a></article>)}</div> : <p className="empty-state compact">{t("noRuleBasedGaps")}</p>}</section>
  </>;
}
