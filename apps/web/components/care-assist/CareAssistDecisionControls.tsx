...
export function CareAssistDecisionControls({ findingId, severity, onSaved }: { findingId: string; severity: string; onSaved: () => void }) {
  ...
  async function decide(decision: "ACCEPT" | "DISMISS" | "SNOOZE" | "RESOLVE") {
    ...
    try {
      await decideCareAssistFinding(findingId, { decision, reason, snoozedUntil });
  ...
  return (
    <div className="data-list">
      ...
      <div className="toolbar">
        <button className="button compact" type="button" onClick={() => void decide("ACCEPT")}>Accept</button>
        ...
        <button className="button secondary compact" type="button" onClick={() => void decide("RESOLVE")}>Resolve</button>
      </div>
