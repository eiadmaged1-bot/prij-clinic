...
test("Round 4 proves recoverability, conflict visibility, session states, and real receptionist denial", async ({ page }, testInfo) => {
  ...
  await receptionistBoundary(page, testInfo, stateVisit);
...
async function saveFailureAndRecovery(page: Page, testInfo: TestInfo, visit: VisitRef) {
  ...
  const handler = async (route: Route) => {
    if (route.request().method() === "PATCH" && !failed) {
      failed = true;
      await route.fulfill({ status: 503, contentType: "application/json", body: JSON.stringify({ message: "Synthetic temporary save outage" }) });
      return;
    }
...
async function versionConflict(page: Page, testInfo: TestInfo, visit: VisitRef) {
  ...
  const external = await patchVisit(page, visit, { revision, assessmentText: `Concurrent server edit ${Date.now()}` });
...
async function partialResource(page: Page, testInfo: TestInfo, visit: VisitRef) {
  ...
  const handler = async (route: Route) => {
    ...
    await route.fulfill({ response, json: { ...body, resourceErrors: [...errors, { resource: "history", message: "Synthetic partial history failure" }] } });
  };
...
async function packetFailure(page: Page, testInfo: TestInfo, visit: VisitRef, status: number, heading: string, screenshot: string) {
  const pattern = packetPattern(visit);
  const handler = async (route: Route) => {
    await route.fulfill({ status, contentType: "application/json", body: JSON.stringify({ message: `Synthetic ${status} visit packet state` }) });
  };
...
async function receptionistBoundary(page: Page, testInfo: TestInfo, visit: VisitRef) {
  ...
  const status = await page.evaluate(async ({ patientId, encounterId }) => {
    ...
    const response = await fetch(`/api/backend/patients/${encodeURIComponent(patientId)}/doctor-visit/${encodeURIComponent(encounterId)}/packet`, { credentials: "include", headers });
...
async function createCockpitVisit(page: Page, marker: string): Promise<VisitRef> {
  ...
  const started = await page.evaluate(async (patientId) => {
    ...
    const response = await fetch(`/api/backend/patients/${encodeURIComponent(patientId)}/doctor-visit/start`, { method: "POST", credentials: "include", headers, body: "{}" });
...
async function patchVisit(page: Page, visit: VisitRef, body: Record<string, unknown>) {
  return page.evaluate(async ({ patientId, encounterId, body: requestBody }) => {
    const token = sessionStorage.getItem("prijClinicToken");
    const headers: Record<string, string> = { "content-type": "application/json", ...(token ? { authorization: `Bearer ${token}` } : {}) };
    const response = await fetch(`/api/backend/patients/${encodeURIComponent(patientId)}/doctor-visit/${encodeURIComponent(encounterId)}`, { method: "PATCH", credentials: "include", headers, body: JSON.stringify(requestBody) });
    return { ok: response.ok, status: response.status, body: await response.json().catch(() => null) };
  }, { ...visit, body });
}

function ref(patient: { id: string; name: string }, encounterId: string): VisitRef {
  return { patientId: patient.id, patientName: patient.name, encounterId, visitUrl: `/patients/${patient.id}/visits/${encounterId}/encounter` };
}
...
function visitPatchPattern(visit: VisitRef) { return `**/api/backend/patients/${visit.patientId}/doctor-visit/${visit.encounterId}`; }
function packetPattern(visit: VisitRef) { return `**/api/backend/patients/${visit.patientId}/doctor-visit/${visit.encounterId}/packet`; }
function escapeRegex(value: string) { return value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"); }

async function captureAndCheck(page: Page, testInfo: TestInfo, name: string) {
  await page.screenshot({ path: testInfo.outputPath(name), fullPage: true });
  const metrics = await page.evaluate(() => ({ root: document.documentElement.scrollWidth, client: document.documentElement.clientWidth, body: document.body.scrollWidth }));
