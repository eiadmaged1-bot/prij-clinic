...
## Part H — Recovery and Behavioral Verification

- The persistent-development incident is documented in `PART_H_DEV_DATABASE_INCIDENT_20260712.md`. The identified `db push --accept-data-loss` targeted `prij_clinic_dev`, not the active clinic database; 18 Guideline columns and 10 indexes were dropped. Recovery was not executed.
- Forward-only migration `20260712073722_part_h_performance_indexes` creates only justified schema indexes. All 46 migrations deployed cleanly to `prij_clinic_test_part_h_chain_final` with no unexpected drift.
- Patient domain ownership is separated across search, lookup, registration, and timeline services. The behavioral architecture suite passed three times on three fresh disposable databases using unique fixtures.
- The patient workspace uses one 16-module registry, lazy heavy-module boundaries, permission-filtered deep links, and browser-history-safe `?module=` routing. No replacement 3,000-line source file remains.
- `encounter.delete` was rejected as an invented permission. Encounter removal uses the existing `encounter.void` policy with backend authorization, a required reason, and audit recording.
- Timeline responses are `{ items, nextCursor, hasMore }`, ordered by event time descending then source type and source ID. Limit defaults to 25 and is capped at 50; at most 1,021 candidates are considered across 20 sources for a maximum-size page.
- Real Playwright instrumentation measured one initial patient-workspace data request, one request on a visible module switch, and two on a timeline deep link. A receptionist made one workspace request and no clinical request.
- The final patient route measured 32.4 kB with 164 kB First Load JS. Heavy patient chunks were verified absent from the initial route chunk list.
- All ten Part H suites, critical Parts A-G regressions, shared/API/web typechecks, the final production build, and post-build bundle verification passed.
- Manual QA Wave 1 is documented but remains pending. Part I, offline sync, and AI work were not started.
