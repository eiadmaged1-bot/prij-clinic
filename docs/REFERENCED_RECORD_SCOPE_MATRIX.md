...
| `/appointments` | `POST` | body `patientId`, optional `doctorId` | Patient, User | `appointment.manage` | Patient branch must be allowed; doctor must belong to allowed branch when supplied | Doctor can create only in own scope when acting as doctor | Appointment patient must be allowed | Enforced by referenced-record helpers | Low | Expanded scope tests |
...
| `/queue/check-in` | `POST` | body `patientId`, optional `appointmentId` | Patient, Appointment, QueueTicket | `queue.manage` | Patient and appointment must be in actor branch | Appointment doctor scope where applicable | Appointment must match patient | Enforced by referenced-record helpers | Low | Expanded scope tests |
...
| `/encounters` | `POST` | body `patientId`, optional `appointmentId` | Patient, Appointment, Encounter | `encounter.create` | Patient/appointment must be in actor branch | Doctor must reference own appointment when appointment is supplied | Appointment must match patient | Enforced by referenced-record helpers | Low | Expanded scope tests |
...
| `/prescriptions` | `POST` | body `patientId`, optional `encounterId` | Patient, Encounter, Prescription | `prescription.create` | Patient/encounter must be in actor branch | Doctor must reference own encounter when supplied | Encounter must match patient | Enforced by referenced-record helpers | Low | Expanded scope tests |
...
| `/investigations/orders` | `POST` | body `patientId`, optional `encounterId` | Patient, Encounter, InvestigationOrder | `investigation.create` | Patient/encounter must be in actor branch | Doctor must reference own encounter when supplied | Encounter must match patient | Enforced by referenced-record helpers | Low | Expanded scope tests |
...
| `/reports` | `POST` | body `patientId`, optional `encounterId`, optional `investigationOrderId` | Patient, Encounter, InvestigationOrder, Report | `report.upload` | Patient/linked records must be in actor branch | Doctor must reference own clinical records where modeled | Linked records must match patient | Enforced by referenced-record helpers | Low | Expanded scope tests |
...
| `/pregnancies` | `POST` | body `patientId` | Patient, Pregnancy | `pregnancy.manage` | Patient branch must be actor branch | Doctor assignment not modeled | Pregnancy must belong to patient | Enforced by referenced-record helpers | Accepted Demo Limitation | Expanded scope tests |
...
| `/ob-ultrasounds` | `POST` | body `patientId`, optional `pregnancyId`, optional `encounterId` | Patient, Pregnancy, Encounter, ObUltrasound | `ob_ultrasound.manage` | Patient/linked records must be actor branch | Encounter doctor-owned where modeled | Linked records must match patient | Enforced by referenced-record helpers | Accepted Demo Limitation | Expanded scope tests |
...
| `/billing/invoices` | `POST` | body `patientId` | Patient, Invoice | `billing.manage` | Patient branch must be actor branch | Not clinical doctor scope | Invoice patient must be allowed | Enforced by referenced-record helpers | Low | Expanded scope tests |
...
| `/ai-drafts` | `POST` | body `patientId`, optional `encounterId` | Patient, Encounter, AiDraft | `ai_draft.request` | Patient/encounter must be actor branch | Encounter doctor-owned where modeled | Encounter must match patient | Enforced by referenced-record helpers; no final clinical write | Low | Expanded AI/scope tests |
...
Critical public-access gaps were not found because protected routes require auth and RBAC. This sprint hardened the high-priority create paths that reference patient-linked records before actor scope is checked. Remaining hardening is focused on patient-to-doctor assignment, exhaustive lower-role positive-path coverage, future state transitions, and production audit/consent controls.

## Test Policy

- Clear public access regressions fail tests.
- Cross-branch create/reference bypasses fail tests.
