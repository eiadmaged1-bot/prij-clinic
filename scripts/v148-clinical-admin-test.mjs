...
const [investigations, ultrasound, encounters, pregnancyService, rbac] = await Promise.all([
...
assert.match(ultrasound, /No automatic diagnosis/);
assert.match(ultrasound, /Patient → Pregnancy → Ultrasound/);
...
assert.match(encounters, /Browse all patients/);
