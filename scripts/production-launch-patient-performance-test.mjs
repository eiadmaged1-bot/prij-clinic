...
const lookup = file("apps/api/src/patients/services/patient-lookup.service.ts");
...
assert(lookup.includes("...branchScope(user)") && lookup.includes("receptionistOnly") && lookup.includes("canReadFinance"), "summary is branch and permission scoped");
assert(lookup.includes("availableActions") && lookup.includes("patient.workspace_summary_read"), "backend policy actions and read audit exist");
