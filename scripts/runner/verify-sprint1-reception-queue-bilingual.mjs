...
for (const needle of ["New Patient", "Returning Patient", "Waiting Line", "Next to doctor", "No patient with doctor", "مريضة جديدة", "مريضة مسجلة", "قائمة الانتظار"]) {
  assert.ok(receptionCopy.includes(needle), "Reception bilingual copy missing " + needle);
}
...
for (const needle of ["data-doctor-handoff-workspace", "ui.pickNext", "ui.open", "ui.continue", "ui.complete", "ui.completeSignedFlow", 'moduleKey: "encounter" | "finish"']) {
  assert.ok(operations.includes(needle), "Bilingual doctor queue workflow missing " + needle);
}
for (const needle of [
  'pickNext: "Pick next"', 'pickNext: "اختيار التالية"',
  'open: "Open"', 'open: "فتح"',
  'continue: "Continue"', 'continue: "متابعة"',
  'complete: "Complete"', 'complete: "إنهاء"',
  'completeSignedFlow: "Complete opens the signed finish workflow',
  'completeSignedFlow: "الإنهاء يفتح مسار الإنهاء والتوقيع'
]) {
  assert.ok(operationsCopy.includes(needle), "Doctor queue bilingual copy missing " + needle);
}
