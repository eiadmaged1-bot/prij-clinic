...
const visitPath = path.join(root, "apps/web/components/clinic/ActiveVisitWorkspace.tsx");
const cssPath = path.join(root, "apps/web/app/globals.css");

let visit = await readFile(visitPath, "utf8");
let css = await readFile(cssPath, "utf8");

const listBefore = '<div className="data-list wide">\n        {results.map((result) => <MedicationCard key={`${result.type}-${result.id}`} result={result} onAdd={() => onAdd(result)} />)}';
const listAfter = '<div className="medication-result-grid wide">\n        {results.map((result) => <MedicationCard key={`${result.type}-${result.id}`} result={result} onAdd={() => onAdd(result)} />)}';
if (!visit.includes(listBefore)) throw new Error("Medication result list anchor not found.");
visit = visit.replace(listBefore, listAfter);
...
if (cardStart < 0 || cardEnd < 0) throw new Error("MedicationCard function boundary not found.");
...
const cssMarker = "/* QA-MED-001 compact medication-result cards */";
if (css.includes(cssMarker)) throw new Error("Medication card CSS already exists; refusing duplicate patch.");
css += `\n\n${cssMarker}
...
console.log("Applied QA-MED-001 compact medication-card patch.");
