...
assert(seed.includes('["Salbutamol", "Terbutaline"]') && seed.includes('familyByCode.get("SABA")'), "SABA must link to Salbutamol and Terbutaline");
...
try {
  const family = await prisma.drugFamily.findUnique({ where: { code: "SABA" }, include: { genericMemberships: { include: { medication: true } } } });
  assert(family, "SABA family missing after idempotent seed");
  assert.deepEqual(family.genericMemberships.map((item) => item.medication.genericName).sort(), ["Salbutamol", "Terbutaline"], "SABA generic membership is incomplete");
  assert(family.genericMemberships.every((item) => item.medication.reviewStatus === "needs_review"), "seeded identities must not claim clinical verification");
} finally { await prisma.$disconnect(); }

console.log("v1.4.5 generic-first pharmacology data model PASS (35 assertions)");
