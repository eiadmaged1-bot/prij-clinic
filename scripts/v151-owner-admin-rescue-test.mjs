...
const [owner, services, investigations, auditPage, auditService, adminController, accountsService, appearance, security, css] = await Promise.all([
...
for (const token of ["Services & Pricing", "data-table", "pagination-row", "admin-editor-drawer", "New service", "Deactivate", "Restore"]) assert.match(services, new RegExp(token));
for (const token of ["Investigation Catalog", "data-table", "pagination-row", "admin-editor-drawer", "Clinical group"]) assert.match(investigations, new RegExp(token));
assert.match(adminController, /@Get\("audit"\)/);
assert.match(auditService, /listPage/);
assert.match(auditService, /skip: \(page - 1\) \* pageSize/);
for (const token of ["server pagination", "Severity", "Resource", "Page \{data.page\}"]) assert.match(auditPage, new RegExp(token));
