...
async function createInvoice(label) {
  ...
  const patient = await request('POST', '/patients', cookie, { medicalRecordNumber: `PAY-${suffix}`, firstName: 'Isolated', lastName: label, phone: `01${Math.floor(Math.random() * 1e9).toString().padStart(9, '0')}` });
  assert.equal(patient.status, 201, patient.data);
  const invoice = await request('POST', '/billing/invoices', cookie, { patientId: JSON.parse(patient.data).id, items: [{ description: 'Concurrency test item', unitAmount: 1000, quantity: 1 }] });
  assert.equal(invoice.status, 201, invoice.data);
  return JSON.parse(invoice.data).id;
}

async function pay(invoiceId, amount) {
  return request('POST', '/billing/payments', cookie, { invoiceId, amount: String(amount), method: 'cash', note: 'Automated isolated concurrency test' });
}
