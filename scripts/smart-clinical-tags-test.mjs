...
const [schema, migration, provenanceMigration, seed, controller, service, patientUi, searchUi] = await Promise.all([
  ...
  readFile("apps/api/prisma/migrations/20260714143000_clinical_tag_provenance_confirmation/migration.sql", "utf8"),
...
for (const field of ["sourceEncounterId", "assignmentType", "doctorConfirmed", "status", "effectiveDate", "resolutionDate", "labelAr", "aliasesArJson"]) {
  assert(schema.includes(field) && provenanceMigration.includes(`"${field}"`), `clinical tag provenance field missing: ${field}`);
}
...
assert(service.includes("termMatches.every") && service.includes("splitTerms") && service.includes("matchingTags"), "multi-tag AND search must be implemented");
assert(service.includes('operator === "OR"') && service.includes('operator === "NOT"'), "AND, OR, and NOT cohort operators must be implemented");
